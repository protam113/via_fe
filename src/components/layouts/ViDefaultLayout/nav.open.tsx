...
const menuItems = [
  ...
  { label: 'Via Art Fair', href: '/vi/via-art-fair' },
  { label: 'Via Atelier', href: '/vi/via-atelier' },
  { label: "Via Prive'", href: '/vi/via-prive' },
  { label: 'Liên hệ', href: '/vi/contact-us' },
];
...
export default function ViNavigationMenu() {
        ...
        <div className="flex flex-col md:flex-row h-full w-full p-8 md:p-12 max-h-screen">
            ...
            <nav className="space-y-2 md:space-y-4">
              {menuItems.map(({ label, href }) => (
                <div key={label} className="py-1">
                  <Link
                    href={href}
                    className="block text-3xl md:text-5xl font-medium text-black hover:underline transition-transform duration-300 hover:translate-x-2 leading-normal"
                    onClick={() => setIsOpen(false)}
                  >
                    {label}
                  </Link>
          ...
          <div className="mt-8 md:mt-0 md:self-end flex flex-col items-start md:items-end space-y-6">
            <div className="text-left md:text-right">
              <h3 className="text-gray-800 mb-2 text-sm">Thông tin liên hệ</h3>
              <p className="mb-1">tomuraleevn@gmail.com</p>
              <p className="mb-1">+84 123 4567889</p>
              <p className="mb-1">24, Đường số 1, An Khánh</p>
              <p>Thành phố Thủ Đức, Thành phố Hồ Chí Minh</p>
            </div>

            <div className="text-left md:text-right">
              <h3 className="text-gray-400 mb-2 text-sm">Mạnh Xã Hội</h3>
